        ...
        for idx_data, d in enumerate(self.loader_test):
            for idx_scale, scale in enumerate(self.scale):
                ...
                for lr, hr, filename in tqdm(d, ncols=80):
                    ...
                    print(f'file={filename[0]}  psnr={psnr}')

                    if math.isnan(psnr):
                        print(f'[NaN FOUND] file={filename[0]}')
                        print(f'  lr shape = {tuple(lr.shape)}')
                        print(f'  hr shape = {tuple(hr.shape)}')
                        print(f'  sr shape = {tuple(sr.shape)}')
                        print(f'  sr has nan = {torch.isnan(sr).any().item()}')
                        print(f'  hr has nan = {torch.isnan(hr).any().item()}')
